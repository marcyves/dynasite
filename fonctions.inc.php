...
function prepareContenu($dossier_menu, $page_active){
	...
	$i =0;
	while($boucle){
		// construit le chemin et le nom du fichier avec un numéro $i
		$fichier_contenu = $dossier_menu."/$i$page_active.menu";
		if(is_readable($fichier_contenu)){
			// Le fichier est trouvé
			// On lit le contenu
			$tmp = file_get_contents($fichier_contenu);
			// On positionne les interrupteurs logiques
			$boucle = false;
			$trouve = true;
		}
		$i++;
